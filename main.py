...
def show_expenses(expenses):
    if not expenses:
        ...
        for expense in expenses:
            print(f"name: {expense['name']} - amount: ${expense['amount']:.2f} - category: {expense['category']} - time:{format_datetime(expense['datetime'])} ")
...
def delete_expense(expenses):
    ...
    for index, expense in enumerate(expenses, start=1):
        print(f"{index}. {expense['name']} - ${expense['amount']:.2f} - {expense['category']}")
    try:
...
def search_expense(expenses):
    ...
    for expense in expenses:
        if keyword in expense['name'].lower():
            print(f"{expense['name']} - ${expense['amount']:.2f} - {expense['category']} - {format_datetime(expense['datetime'])}")
            found = True
    if not found:
